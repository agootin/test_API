...
result = session.query(tables.Person.person_id, tables.Person.person_name)\
    .filter(tables.Person.person_id > 9999999).all()

print(result)

# result = session.query(tables.Person.person_name).filter(tables.Person.person_id.in_(person_ids)).all()
# print(result)
